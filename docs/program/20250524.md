...
配合Meilisearch的相关插件在上一步已经安装完毕了，所以可以直接进入Flarum后台，启用Scout Search扩展。Driver选择Meilisearch；Index name prefix (optional)随便填，我填了`blc`；Meilisearch Host填写主机端口，这里因为我没改默认端口所以保持默认即可；Meilisearch Key填写为上一步获取的密钥。然后应用就可以了。
